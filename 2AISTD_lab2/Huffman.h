...
private:
	...
	{
	public:
		Node(int freq = 0, Node* Left = nullptr, Node* Right = nullptr, char lett = '\n')
		{
			frequence = freq;
			letter = lett;
			left = Left;
...
public:
	...
	Huffman(char* _str)
	{
		...
		if (sizeStr > 1)
		{
			...
			for (size_t i = 0; (i < Cl) && (!pq.isEmpty()); i++)
			{
				...
				BN_1->parent = root;
				BN_2->parent = root;
				pq.push_el(root, root->frequence);  //return it to the queue
				BI = pq.tail_prior();
				BN_1 = pq.pop();
			}
	...
	bool* Code(int& codeSize)
	{
		...
		codeSize = 0;
		for (int i = 0; str[i] != '\n'; i++) {
			codeSize += codeLetter.Find_Element(str[i])->size;
		}
		sizeReadyCode = codeSize;
		readyCode = new bool[codeSize];
		size_t indexStr = 0;
		for (int i = 0; str[i] != '\n'; i++) {
			copyStr = codeLetter.Find_Element(str[i])->str;  //we find a match in the tree
			for (size_t j = 0; j < codeLetter.Find_Element(str[i])->size; j++) {
